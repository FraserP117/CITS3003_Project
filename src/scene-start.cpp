...
// rotate ADDED:
mat4 rotate; // the rotaton matrix of SceneObject.angles[i] arround each axis:
...
// ADDED for part C:
static void Identity(vec2 vec) {
    sceneObjs[toolObj].loc[0] += 0;
    sceneObjs[toolObj].loc[1] += 0;
    sceneObjs[toolObj].loc[2] += 0;
}

// ADDED for Part C:
static void adjustLocY(vec2 y) {
    sceneObjs[toolObj].loc[1] += y[0];
    sceneObjs[toolObj].loc[1] += y[1];
}
...
// ADDED: for part C
/*
the ambient or diffuse light of the object can be interactively
changed by dragging the left mouse button horizontally or vertically;
*/
static void adjustAmbientandDiffuse(vec2 by) {
    sceneObjs[toolObj].ambient += by[0]; // hoz
    sceneObjs[toolObj].diffuse += by[1]; // vert
}
...
*/
static void adjustSpecularityandShine(vec2 by) {
    sceneObjs[toolObj].specular += by[0]; // hoz
    sceneObjs[toolObj].shine += by[1]; // vert
}
...
/*
the position of the light source can be modified by dragging the mouse button.
For this functionality, you are suggested to use
...
void reshape(int width, int height) {
    ...
    GLfloat top = 0;
    GLfloat bottom = 0;
    // ADDED nearDist for part D: Reduced nearDist by factor
    GLfloat nearDist = 0.02;
    GLfloat far = 100.0;

    // ADDED for part E:
    if (width < height) {
      GLfloat ratio_1 = (float)height/(float)width;
      left = 1.0;
      right = 1.0;
      bottom = ratio_1;
      top = ratio_1;
    } else {
      GLfloat ratio_2 = (float)width/(float)height;
      left = ratio_2;
      right = ratio_2;
      top = 1.0;
      bottom = 1.0;
    ...
    projection = Frustum(-nearDist * left,
                         nearDist * right,
                         -nearDist * bottom,
                         nearDist * top,
                         nearDist,
