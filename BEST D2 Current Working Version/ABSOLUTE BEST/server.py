    ...
    * When the game is finished, if there are enough players available the
    ...
    def startGame(self):
        ...
        while True:
            ...
            server.gameStart(board,live_id_nums)
    ...
    def gameStart(self,board,live_id_nums):
        global all_clients
        global current_players
        global started
        global started
        ...
        while len(live_id_nums)>1: #step 5
            ...
            print("here")
            for i in all_clients:
                print(i.client_address)
                #step 5 a
                i.connection.send(tiles.MessagePlayerTurn(current_idnum).pack())#this sends players names
            print("here1")
            ...
            if isinstance(msg, tiles.MessagePlaceTile):
                #move = (msg, tiles.MessagePlaceTile)
                #move = ((msg.x, msg.y, msg.tileid, msg.rotation, msg.idnum), 8)
                moves.append(msg)
                #self.movesQueue.put(move)
                if board.set_tile(msg.x, msg.y, msg.tileid, msg.rotation, msg.idnum):
                    ...
                    for msg in positionupdates:
                        moves.append(msg)
                        #print("my connection address is " + current_address)
                        print("sending to ")
                        for i in all_clients:
                            #print("this connectiong: "+i.client_address)
                            i.connection.send(msg.pack())
                        print("done sending")
                    print("gothere")
                    for e in eliminated:
...
#                         return
                ...
                if not board.have_player_position(msg.idnum):
                    #move = (msg, tiles.MessageMoveToken)
                    #move = ((msg.idnum, msg.x, msg.y, msg.position), 9)
                    #moves.append(msg)
                    #self.movesQueue.put(move)
...
#         thread.start()
    ...
    if server.server_socket in readable:
        ...
        if(started == -1 and len(all_clients)==2):
