...
class Server:
    ...
    def start_server(self):
        ...
        while True:
            # handle each newely connected client indipendently:
            read, dummy_1, dummy_2 = select.select(all_connections, [], [])
            # accept the client:
            CLIENT_connection, CLIENT_address = server_socket.accept()
            # add this client to the list of clients:
            all_connections.append(CLIENT_connection)
            # add this client's address to the list of client addresses:
            all_addresses.append(CLIENT_address)

            # create the threads:
            for conn, addr in zip(all_connections, all_addresses):
                thread = threading.Thread(
